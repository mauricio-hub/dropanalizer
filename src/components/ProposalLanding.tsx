...
export default function ProposalLanding({ proposal, version }: ProposalLandingProps) {
  ...
  if (content.contentType === 'dropshipping') {
  ...
  return <B2BLanding proposal={proposal} version={version as any} />
}

function B2BLanding({ proposal, version }: { proposal: ProposalLandingProps['proposal']; version: { id: string; content: ProposalContent } }) {
  const content = version.content
  const sectionRefs = useRef<Record<string, { element: HTMLElement; startTime: number | null; totalTime: number }>>({})

  useEffect(() => {
    const sections = ['hero', 'scope', 'deliverables', 'timeline', 'pricing', 'cta']
  ...
  const handleClick = (section: string) => trackClick(version.id, section)
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="relative overflow-hidden" data-section="hero" onClick={() => handleClick('hero')}>
        ...
        <div className="relative mx-auto max-w-5xl px-6 py-24 md:py-32">
          ...
          <p className="text-lg text-text-secondary max-w-2xl">Professional proposal crafted for your success</p>
      ...
      <div className="mx-auto max-w-5xl px-6 py-16 space-y-20">
        {scope && (
          <section className="grid md:grid-cols-[1fr_1fr] gap-12 items-center" data-section="scope" onClick={() => handleClick('scope')}>
            <div>
        ...
        {deliverables.length > 0 && (
          <section data-section="deliverables" onClick={() => handleClick('deliverables')}>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {deliverables.map((item: string, idx: number) => (
                <div key={idx} className="group bg-surface rounded-xl border border-white/[0.08] p-6 hover:border-accent/30 transition-all">
        ...
        {timeline.length > 0 && (
          <section data-section="timeline" onClick={() => handleClick('timeline')}>
            <h2 className="text-2xl md:text-3xl font-bold text-text-primary mb-8">Timeline</h2>
            <div className="space-y-4">
              {timeline.map((phase: any, idx: number) => (
                <div key={idx} className="bg-surface rounded-xl border border-white/[0.08] p-6 hover:border-accent/30 transition-all">
        ...
        <section className="bg-surface rounded-2xl border border-white/[0.08] p-8 md:p-12" data-section="pricing" onClick={() => handleClick('pricing')}>
          <div className="grid md:grid-cols-2 gap-12">
            ...
            <div className="flex flex-col justify-center">
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-5xl font-bold text-text-primary">{pricing.total?.toLocaleString()}</span>
        ...
        <section className="text-center py-12 border-y border-white/[0.06]" data-section="cta" onClick={() => handleClick('cta-section')}>
          <h2 className="text-2xl font-bold text-text-primary mb-4">Ready to move forward?</h2>
          <p className="text-text-secondary mb-8 max-w-2xl mx-auto">Contact us to discuss details and get started on this project.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button
              onClick={() => handleClick('accept-proposal')}
              className="bg-accent hover:bg-accent-hover text-white font-semibold py-3 px-8 rounded-lg transition-colors"
            >
              Accept Proposal
            </button>
            <button
              onClick={() => handleClick('request-changes')}
